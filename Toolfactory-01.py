...
class ToolFactory:
    ...
    def generate_tool(self, tool_description):
        """Uses LLM to generate code for a tool based on the description."""
        prompt = f"""
        Generate a Python function that performs the following task:
        {tool_description}
        
        Provide the complete code for the function.
        """
        response = self.llm.invoke(prompt)
        tool_code = response.content.strip()
        
        # Execute the generated code and store the function in the dictionary
        exec(tool_code, globals())
        function_name = tool_code.split('def ')[1].split('(')[0]
        self.generated_tools[function_name] = eval(function_name)
...
class AIAgent:
    def __init__(self):
        self.llm = llm
        self.tool_factory = ToolFactory(llm)
    ...
    def extract_tools(self, roles):
        ...
        for role in roles.values():
            ...
            if "tool" in role.lower():
                tool_description = role.split("tool:", 1)[1].strip()
                tool_name = self.tool_factory.generate_tool(tool_description)
